...
@implementation SHQRScannerHelper
...
+(UIImage *)scaleImage:(CIImage *)ciImage withSideLength:(CGFloat)sideLength{
    ...
    CGSize originaSize = temp.size;
    CGFloat scale = MIN(sideLength/originaSize.width, sideLength/originaSize.height);
    //按照比例计算缩放后的宽高
    size_t scaledWidth = originaSize.width * scale;
    size_t scaledHeight = originaSize.height * scale;
...
+(NSString *)recognizeQRCodeFromImage:(UIImage *)image{
    ...
    //第一个是最精准的
    CIQRCodeFeature * feature = [resultArr firstObject];
...
+(UIImage *)changeColorForQRImage:(UIImage *)image backgroundColor:(UIColor *)backgroundColor frontColor:(UIColor *)frontColor{
    ...
    //缩放为原来的尺寸
    UIImage * scaledImage = [SHQRScannerHelper scaleImage:colorFilter.outputImage withSideLength:image.size.width];
...
+(UIImage *)compposeQRCodeImage:(UIImage *)codeImage withImage:(UIImage *)image withImageSideLength:(CGFloat)sideLength{
    ...
    //绘制image到codeImage中心
    [image drawInRect:CGRectMake((codeImageWidth - sideLength)/2, (codeImageHeight - sideLength)/2, sideLength, sideLength)];
